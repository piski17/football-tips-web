...
export interface OddsSummary {
  ...
  goalsOverUnder?: Record<string, { over: number; under: number }>; // kľúč = hranica, napr. "2.5"
}

/**
 * Načíta kurzy od stávkových spoločností pre daný zápas a spriemeruje ich
 * naprieč všetkými dostupnými spoločnosťami (API-Football väčšinou ponúka
 * medzinárodné spoločnosti - Bet365, Unibet a podobne, nie nutne slovenské/
 * české). Kurzy bývajú k dispozícii až bližšie k výkopu, pre vzdialenejšie
 * zápasy môže vrátiť null.
 */
export async function getFixtureOddsSummary(fixtureId: number): Promise<OddsSummary | null> {
  try {
    const res = await client().get("/odds", { params: { fixture: fixtureId } });
    checkApiErrors(res.data);

    const bookmakers: any[] = res.data?.response?.[0]?.bookmakers ?? [];
    if (bookmakers.length === 0) return null;

    const mwHome: number[] = [];
    const mwDraw: number[] = [];
    const mwAway: number[] = [];
    const dcHomeDraw: number[] = [];
    const dcDrawAway: number[] = [];
    const dcHomeAway: number[] = [];
    const bttsYes: number[] = [];
    const bttsNo: number[] = [];
    const goalsOU: Record<string, { over: number[]; under: number[] }> = {};
      ...
      for (const bet of bm.bets ?? []) {
        ...
        if (name === "match winner") {
          for (const v of values) {
            ...
            if (Number.isNaN(odd)) continue;
            if (v.value === "Home") mwHome.push(odd);
            else if (v.value === "Draw") mwDraw.push(odd);
            else if (v.value === "Away") mwAway.push(odd);
          }
        } else if (name === "double chance") {
          for (const v of values) {
            const odd = parseFloat(v.odd);
            if (Number.isNaN(odd)) continue;
            if (v.value === "Home/Draw") dcHomeDraw.push(odd);
            else if (v.value === "Draw/Away") dcDrawAway.push(odd);
            else if (v.value === "Home/Away") dcHomeAway.push(odd);
          }
        } else if (name === "both teams score") {
          for (const v of values) {
            const odd = parseFloat(v.odd);
            if (Number.isNaN(odd)) continue;
            if (v.value === "Yes") bttsYes.push(odd);
            else if (v.value === "No") bttsNo.push(odd);
          }
        } else if (name === "goals over/under") {
          for (const v of values) {
            const odd = parseFloat(v.odd);
            if (Number.isNaN(odd)) continue;
            const m = /^(Over|Under)\s+([\d.]+)$/.exec(String(v.value ?? ""));
            if (!m) continue;
            const line = m[2];
            if (!goalsOU[line]) goalsOU[line] = { over: [], under: [] };
            if (m[1] === "Over") goalsOU[line].over.push(odd);
            else goalsOU[line].under.push(odd);
          }
        }
      }
    }
    ...
    if (mwHome.length || mwDraw.length || mwAway.length) {
      summary.matchWinner = { home: avg(mwHome) ?? 0, draw: avg(mwDraw) ?? 0, away: avg(mwAway) ?? 0 };
    }
    if (dcHomeDraw.length || dcDrawAway.length || dcHomeAway.length) {
      summary.doubleChance = {
        homeDraw: avg(dcHomeDraw) ?? 0,
        drawAway: avg(dcDrawAway) ?? 0,
        homeAway: avg(dcHomeAway) ?? 0,
      };
    }
    if (bttsYes.length || bttsNo.length) {
      summary.bothTeamsScore = { yes: avg(bttsYes) ?? 0, no: avg(bttsNo) ?? 0 };
    }
    if (Object.keys(goalsOU).length > 0) {
      summary.goalsOverUnder = {};
      for (const [line, vals] of Object.entries(goalsOU)) {
        summary.goalsOverUnder[line] = { over: avg(vals.over) ?? 0, under: avg(vals.under) ?? 0 };
      }
    }
